...
# Lazy load pipenv setup to improve shell startup performance
__pipenv_lazy_load() {
  # Remove the lazy loading function
  unfunction pipenv 2>/dev/null
  
  # Define pipenv completion
  _pipenv() {
    eval $(env COMMANDLINE="${words[1, $CURRENT]}" _PIPENV_COMPLETE=complete-zsh pipenv)
  }
  compdef _pipenv pipenv
  
  # Define automatic pipenv shell activation/deactivation
  _togglePipenvShell() {
...
# Create lazy-loading wrapper for pipenv
if (( $+commands[pipenv] )); then
  pipenv() { __pipenv_lazy_load "$@" }
fi
